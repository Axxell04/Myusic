...
export class ManagerFiles {
  ...
  async check (localURI) {
    const infoFile = await FileSystem.getInfoAsync(localURI);

    if (infoFile.exists) {return true} else {return false}
  };

  async download (remoteURI="", localURI="") {
    ...
    if ((await this.check(localURI)) === false) {
...
export function OldManagerFiles() {
