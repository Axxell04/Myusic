...
export function ModalYT() {
  ...
  // webViewReff.injectJavaScript()
  const webViewRef = useRef(null);

  const stopVideoScript = `
    document.querySelectorAll('video, audio').forEach(media => {
      media.pause();  // Pausa el video o audio// Opcional: Reinicia el video/audio al comienzo
    });
  `
  ...
  useEffect(() => {
    if (!modalYTIsVisible) {
      // webViewRef.current.reload();
      webViewRef.current.injectJavaScript(stopVideoScript);
